...
3. Paste:
   ```
   https://github.com/panchuel/bettertabs-unity.git
   ```
...
- **Drag and drop** — drag any folder or asset from the Project panel directly into the window.
- **Ctrl+T** — pins the asset currently selected in the Project panel as a new tab (works without focusing the BetterTabs window first).
- **`+` button** — adds the currently selected asset as a tab when it is not already open.
